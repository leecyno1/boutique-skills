...
description: |
  Use when the user wants a reusable Xiaohongshu topic/title bank instead of asking AI to freely brainstorm, especially to map topics to user needs such as emotional resonance, growth proof, new insight, learning value, or identity expression, and to assign each topic a growth job or fallback template. Trigger phrases: "选题库", "标题公式", "每天发什么", "今天没灵感", "问题经验观点", "拉新", "建立信任", "强化标签", "系列内容", "批量选题", "爆款需求", "XHS topic bank", "title formulas".
---
...
内容节奏文章补充了选题在增长循环中的职责：先把题目标成**拉新、建立信任或强化标签**的主要任务，再决定它要留下方向、标签或信任资产。没有灵感时，使用“一个问题（用户痛点）→ 一个经验（自己的经历/动作）→ 一个观点（自己的判断）”作为启动模板；模板只是骨架，必须补真实场景，不能生成脱离经历的空泛鸡汤。
...
6. 用户今天没灵感，但想用真实经历快速启动一条内容。
7. 用户想把一批题目分成拉新、建立信任和强化标签三类。
...
6. **标注增长循环任务**
   - 为每个选题标注主要任务：`acquisition`（拉新）、`trust`（建立信任）或 `label`（强化标签），并写清它要沉淀的方向、标签或信任资产。
   - 完成标准: 一批选题不能全部只追求曝光；至少说明下一轮如何把拉新、信任和标签串起来。

7. **无灵感时使用问题—经验—观点模板**
   - 先写一个用户痛点问题，再填一个自己亲历的经验/动作，最后给出一个与常见说法有差异的观点。
   - 完成标准: 模板必须包含真实场景、具体动作和可核验边界；没有真实素材时标记 `needs_source`，不编造个人经历。
...
- 一批选题全部追求拉新，没有建立信任或强化标签的内容。
- 无灵感模板只写“问题—经验—观点”的空壳，没有真实经历、动作和证据边界。
- 把“做系列”误解成重复标题，未说明系列要强化哪个标签或沉淀什么认知资产。
...
- **内容节奏融合**: 2026-08-01，新增增长循环任务与问题—经验—观点兜底模板。
